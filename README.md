...
`.github/workflows/post.yml` は毎日 11:00 JST に実行されます。新しい投稿済み ID が記録された場合は、`data/posted.json` を自動で commit します。
...
MIXI2_MATH_CT_TOKEN_URL
MIXI2_MATH_CT_API_ADDRESS
```

環境変数の prefix は、arXiv カテゴリ名の `.` を `_` に置き換えて大文字化したものです。たとえば `math.CT` は `MIXI2_MATH_CT` になります。
